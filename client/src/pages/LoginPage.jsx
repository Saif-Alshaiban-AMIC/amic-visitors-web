...
export default function LoginPage({ onLogin }) {
  ...
  return (
    <Box sx={{
      ...
      <Paper elevation={3} sx={{ width: '100%', maxWidth: 400, overflow: 'hidden' }}>
        ...
        <Box component="form" onSubmit={handleSubmit}
          sx={{ p: '28px 32px', display: 'flex', flexDirection: 'column', gap: 2 }}>
          {error && <Alert severity="error">{error}</Alert>}

          <TextField label="Username" value={form.username} onChange={set('username')}
            required autoFocus autoComplete="username" sx={fieldSx} />

          <TextField label="Password" type="password" value={form.password}
            onChange={set('password')} required autoComplete="current-password" sx={fieldSx} />
